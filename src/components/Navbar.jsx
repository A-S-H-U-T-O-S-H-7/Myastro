...
import { showLoginPopup, logout } from "@/redux/slices/userSlice";
import { useDispatch, useSelector } from 'react-redux';
const Navbar = () => {
  const { isAuthenticated, details } = useSelector((state) => state.user);
  ...
  return (
    <nav className="bg-white/60 backdrop-blur-md text-[#212529] sticky top-0 z-50 border-b-2 border-solid border-neutral-200">
      ...
      <div
        className={`fixed top-0 left-0 h-full bg-white shadow-lg transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"
