...
const OtherDetailsForm = ({ initialValues, activeTab, setActiveTab, astrologerid }) => {
  const [loader, setLoader] = useState(false);
  const handleSubmit = async (values) => {
    console.log(values)
    try {
      setLoader(true);
      const data = await fetch(`${ENV.API_URL}/admin/astrologer/other-detail-update/${astrologerid}`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          "step": 4,
          "onboardyou": values.whyOnboard,
          "mainsourceincome": values.mainsourceincome,
          "qualification": values.qualification,
          "learastrology": values.learnAstrology,
          "instagram": values.instagram,
          "facebook": values.facebook,
          "youtube": values.youtube,
          "linkedIn": values.linkedin,
          "website": values.website,
          "referastro": values.referredBySomeone,
          "minimumcharges": values.minCharges,
          "minimumearningmyastro": values.minEarnings,
          "currentlyworkingfulltimejob": values.fulltimeJob,
